...
def commit_new_build(prod_name, build_name, app_settings, exalt_version=""):
    ...
    if build_name == "Client":
        if exalt_version != "":
            ...
            commit_msg_lines.append(f"ERROR: Couldn't extraction Exalt version - must be manually updated!")
    ...
    diff_count = len(repo.index.diff("HEAD"))
    if diff_count == 0:
    ...
    logger.log(logging.INFO, f"Comitting {diff_count} changed files...")
